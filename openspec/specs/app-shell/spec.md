...
The wrapping application: project bootstrap (Flask + Jinja, no JS), directory layout with enforced import boundaries, `create_app` factory, configuration classes, blueprint registration, tooling (ruff + mypy + pytest), and developer documentation. The shell is what a future board blueprint plugs into.
