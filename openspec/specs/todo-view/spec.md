...
The v1 user-facing surface: a server-rendered Jinja page that lets the user add, toggle, and delete tasks via plain HTML form posts (Post/Redirect/Get, no JavaScript). Structured so a future board view can coexist as a sibling without changing these templates or routes.
